...
-(void)getNetData {
    NSString *url = [NSString stringWithFormat:kHeaderInfoUrl,self.infoId];
    __weak typeof(self) weakSelf = self;
    [weakSelf.manager GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        if (responseObject) {
            SubjectModel *model = [[SubjectModel alloc] initWithData:responseObject error:nil];
...
-(void)firstDownload {
    ...
    self.isLoadMore = NO;
    self.page = 1;
    NSString *url = [NSString stringWithFormat:kSubjectUrl,self.infoId,20*(self.page-1),20*self.page];
...
-(void)createRefreshView {
    ...
    [weakSelf.tableView addRefreshHeaderViewWithAniViewClass:[JHRefreshCommonAniView class] beginRefresh:^{
        ...
        NSString *url = [NSString stringWithFormat:kSubjectUrl,weakSelf.infoId,20*(weakSelf.page-1),20*weakSelf.page];
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
        return self.data.count+self.dataArr.count;
}
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {
        static NSString *identifier = @"subject";
        SubjectTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
        if (!cell) {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"SubjectTableViewCell" owner:nil options:nil] lastObject];
        }
        SubjectModel *model = self.data[0];
        ...
        Cources *model = self.dataArr[indexPath.row-1];
        [cell showDataWithModel:model];
        return cell;
    }
}
